...
const getHome = async function ({ headers }) {
    ...
    let requestURL = 'https://juris.pjud.cl/busqueda?Compendio_de_Salud_Corte_de_Apelaciones';
...
const searchByDate = async function ({ start, end, date, count = 50, page = 1, canonicalURL, headers }) {
    console.log(`searchCorteSupremaByDate - ${date && date.format("YYYY-MM-DD") || start && start.format("YYYY-MM-DD") + "-" + end && end.format("YYYY-MM-DD")} (page=${page}, count=${count}`);
    ...
    let customHeaders = {
        ...
        "Pragma": "no-cache",
        "Referer": "https://juris.pjud.cl/busqueda?Compendio_de_Salud_Corte_de_Apelaciones",
    ...
    let filters = `{"rol":"","era":"","fec_desde":"${start && start.format("YYYY-MM-DD") || ""}","fec_hasta":"${end && end.format("YYYY-MM-DD") || ""}","tipo_norma":"","num_norma":"","num_art":"","num_inciso":"","todas":"","algunas":"","excluir":"","literal":"","proximidad":"","distancia":"","analisis_s":"11","submaterias":"","facetas_seleccionadas":[${date && `{"nombre":"fec_sentencia_sup_dt","valores":["${date.format("YYYY-MM-DD") || ""}"]}` || ""}],"filtros_omnibox":[{"categoria":"TEXTO","valores":[""]}],"ids_comunas_seleccionadas_mapa":[]}`;
    // throw JSON.stringify({date, start, end, filters}, null, 1)
    body.append('filtros', filters);
...
const clickVerSentencia = async function ({ id, canonicalURL, headers }) {
    ...
    let content = await clickTabTribunales({
        datos,
        headers,
        canonicalURL: canonicalURL + "&tab=tribunales"
    });
...
const clickTabTribunales = async function ({ datos, canonicalURL, headers }) {
    ...
    let responsePage = await fetchPage({ canonicalURL, requestURL, requestOptions, retainSession: true });
    await validateSentenciaRequest({ responsePage, isContent: true, option: 2, canBeEmpty: true });
    return responsePage;
};
...
async function fetchURL({ canonicalURL, headers }) {
    ...
    const rangeMatch = canonicalURL.match(/[\?&]from=(\d{4}-\d{2}-\d{2})&to=(\d{4}-\d{2}-\d{2})(&page=(\d+))?$/i);
    ...
    if (match) {
        ...
        let start = moment(rangeMatch[1]);
        let end = moment(rangeMatch[2]);
        let page = rangeMatch[4] ? parseInt(rangeMatch[4]) : 1;
        return [await searchByDate({ start, end, page, canonicalURL, headers })]
    } else if (idMatch) {
        let id = idMatch[1];
...
const testCrawler = async function () {
    let canonicalURLs = [
        "https://juris.pjud.cl/busqueda?Compendio_de_Salud_Corte_de_Apelaciones&from=2013-01-01&to=2023-10-19&page=2",
        // "https://juris.pjud.cl/busqueda?Compendio_de_Salud_Corte_de_Apelaciones&id=50738856"
    ];
