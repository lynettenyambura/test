...
const getHomeCorteSuprema = async function ({ headers }) {
    ...
    let requestURL = 'https://juris.pjud.cl/busqueda?Buscador_Jurisprudencial_de_la_Corte_Suprema';
    let responsePage = await fetchPage({ canonicalURL: requestURL, requestOptions });
    setSharedVariable('corte-suprema-home', true);
    ...
    setSharedVariable('corte-suprema-buscador', id_buscador_activo);
    setSharedVariable('corte-suprema-token', token);
    responsePage.response = new fetch.Response(html, responsePage.response);
    return responsePage;
};


const searchCorteSupremaByDate = async function ({ date, count = 50, page = 1, canonicalURL, headers }) {
    console.log(`searchCorteSupremaByDate - ${date.format("YYYY-MM-DD")} (page=${page}, count=${count}`);
    if (!getSharedVariable('corte-suprema-home')) await getHomeCorteSuprema({ headers });
    let customHeaders = {
        "Cache-Control": "no-cache",
        "DNT": "1",
        "Origin": "https://juris.pjud.cl",
        "Pragma": "no-cache",
        "Referer": "https://juris.pjud.cl/busqueda?Buscador_Jurisprudencial_de_la_Corte_Suprema",
    ...
    body.append('_token', getSharedVariable('corte-suprema-token'));
    body.append('id_buscador', getSharedVariable('corte-suprema-buscador'));
...
const validateSentenciaRequest = async function ({ responsePage, isContent = false, canBeEmpty = false, option }) {
    ...
    else if (/CSRF token/i.test(json) || (!isContent && !/"response":/i.test(json) || isContent && !/"(contenido|texto_sentencia)":/i.test(json)))
        throw new InvalidResponse();
...
const clickVerSentencia = async function ({ id, canonicalURL, headers }) {
    ...
    let content2 = await clickTabTribunales({ datos, headers, canonicalURL: canonicalURL + "&tab=tribunales" });
    return [responsePage, content, content2];
};


const clickTabCorteApelaciones = async function ({ datos, canonicalURL, headers }) {
    console.log(`clickTabCorteApelaciones - ${datos.id}`);
    let customHeaders = {
        "Cache-Control": "no-cache",
        "DNT": "1",
        "Origin": "https://juris.pjud.cl",
...
const clickTabTribunales = async function ({ datos, canonicalURL, headers }) {
...
async function fetchURL({ canonicalURL, headers }) {
    ...
    if (match) {
        ...
        return [await searchCorteSupremaByDate({ date, page, canonicalURL, headers })]
    } else if (idMatch) {
        let id = idMatch[1];
...
const testCrawler = async function () {
    let canonicalURLs = [
        // `https://juris.pjud.cl/busqueda?Buscador_Jurisprudencial_de_la_Corte_Suprema&date=2023-01-03`,
        // `https://juris.pjud.cl/busqueda?Buscador_Jurisprudencial_de_la_Corte_Suprema&id=3175387`,
        // `https://juris.pjud.cl/busqueda/documentos?opcion=1&b64=0&id_sentencia=5804612&tipo_instancia_id=cs&tipo_instancia_a_cargar=cs`,
        // `https://juris.pjud.cl/busqueda/documentos?opcion=2&b64=0&id_sentencia=38865663&tipo_instancia_id=ca&tipo_instancia_a_cargar=ca`
        "https://juris.pjud.cl/busqueda?Buscador_Jurisprudencial_de_la_Corte_Suprema&id=3290123"
    ];
    ...
    for (let i = 0; i < canonicalURLs.length; i++) {
        ...
        for (let i = 0; i < responses.length; i++) {
            ...
            fileName = fileName.replace("Buscador_Jurisprudencial_de_la_Corte_Suprema_id_", "");
            let type = responsePage.response.headers.get('content-type');
            ...
            const filePath = path.join(currentDir, '/../pdf', `${fileName}.${ext}`)
