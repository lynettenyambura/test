...
const getHome = async function ({headers}) {
    ...
    let requestURL = 'https://juris.pjud.cl/busqueda?Buscador_Corte_de_Apelaciones';
...
const searchByDate = async function ({date, count = 50, page = 1, canonicalURL, headers}) {
    ...
    let customHeaders = {
        ...
        "Pragma": "no-cache",
        "Referer": "https://juris.pjud.cl/busqueda?Buscador_Corte_de_Apelaciones",
...
const clickVerSentencia = async function ({id, canonicalURL, headers}) {
    ...
    let content = await clickTabCorteSuprema({
        datos,
        headers,
        canonicalURL: canonicalURL + "&tab=corteSuprema"
    });
