...
const gistURL = `https://gist.githubusercontent.com/tindase/6dd272fc9fa5e3facc75c1f0979325ac/raw/b1eb5fb92360e5e1e1d5dc78ff521ddab41e7da4/png_dates.js`;
...
function parsePage({responseBody, URL, html, referer}) {
    ...
    let mother = {
        ...
        year: null,
        class: "Act",
        htmlIsModified: false,
        changedArticleTitles: null
    };
    ...
    $("record").each(function (i) {
        ...
        if (/Name.*of.*Legislation/i.test(_class + " " + level)) {
            ...
            child.URI = `${mother.URI}/consolidatedText`;
    ...
    /***
     *
     * TODO REMOVE THIS TO PROCESS ENTIRE DATASET
     *
     *
     */

    if (!mother.htmlIsModified) {
        return [];
    }
...
const parserTest = function () {
    const fs = require("fs");
    let buffer = fs.readFileSync(__dirname + "/../pdf/foot.xml");
    buffer = parsePage({
        responseBody: {content: buffer},
        URL: "http://ftp.vlex.com/niumedia/Source%20files/split-xmls-with-titles/pg_leg_n/472.xml"
    });
